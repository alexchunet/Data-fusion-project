capture log close
log using "D:\SOUTH ASIA MICRO DATABASE\05.projects_requ\02.FY20_projects\ARG Averages\ARG Averages.txt", replace text
/*==================================================
...
cd "${hostdrive}\SOUTH ASIA MICRO DATABASE\05.projects_requ\02.FY20_projects\ARG Averages\" 


*----------0.2: Blank template

gen Year = .
save "ARG Averages.dta", replace
...
foreach year in 2012 2013 2014 2015 2016 2017 2018 {
	foreach quarter in Q01 Q02 Q03 Q04{
		...
		display "Argentina `year' PNADC `quarter'"
	 	...
	 	foreach var of local variables{
			...
			* By urban/rural
			clear all
			cap: dlw, coun(ARG) y(`year') t(LABLAC-01) mod(ALL) verm(01) vera(02) sur(PNADC) per(`quarter') clear
				if (_rc) continue
			...
			append using "ARG Averages.dta", force
			save         "ARG Averages.dta", replace
...
export excel using "ARG Averages.xlsx", firstrow(variables) replace
