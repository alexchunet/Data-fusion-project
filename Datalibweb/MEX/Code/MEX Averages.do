capture log close
log using "D:\SOUTH ASIA MICRO DATABASE\05.projects_requ\02.FY20_projects\MEX Averages\MEX Averages.txt", replace text
/*==================================================
...
cd "${hostdrive}\SOUTH ASIA MICRO DATABASE\05.projects_requ\02.FY20_projects\MEX Averages\" 


*----------0.2: Blank template

gen Year = .
save "MEX Averages.dta", replace
...
foreach year in 2012 2013 2014 2015 2016 2017 2018 {
	foreach quarter in Q01 Q02 Q03 Q04{
		...
		display "Mexico `year' PNADC `quarter'"
	 	...
	 	foreach var of local variables{
			...
			* By urban/rural
			clear all
			cap: dlw, coun(MEX) y(`year') t(LABLAC-01) mod(ALL) verm(01) vera(02) sur(PNADC) per(`quarter') clear
				if (_rc) continue
			...
			append using "MEX Averages.dta", force
			save         "MEX Averages.dta", replace
...
export excel using "MEX Averages.xlsx", firstrow(variables) replace
