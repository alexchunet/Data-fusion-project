capture log close
log using "D:\SOUTH ASIA MICRO DATABASE\05.projects_requ\02.FY20_projects\BRA Averages\BRA Averages.txt", replace text
/*==================================================
...
cd "${hostdrive}\SOUTH ASIA MICRO DATABASE\05.projects_requ\02.FY20_projects\BRA Averages\" 


*----------0.2: Blank template

gen Year = .
save "BRA Averages.dta", replace
...
foreach year in 2012 2013 2014 2015 2016 2017 2018 {
	foreach quarter in Q01 Q02 Q03 Q04{
		...
		display "Brasil `year' PNADC `quarter'"
	 	...
	 	foreach var of local variables{
			...
			* By urban/rural
			clear all
			cap: dlw, coun(BRA) y(`year') t(LABLAC-01) mod(ALL) verm(01) vera(02) sur(PNADC) per(`quarter') clear
				if (_rc) continue
			...
			append using "BRA Averages.dta", force
			save         "BRA Averages.dta", replace
...
export excel using "BRA Averages.xlsx", firstrow(variables) replace
